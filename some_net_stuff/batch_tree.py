...
class BatchTree(object):
    ...
    class Node(object):
        ...
        def expand_or_add_child(self, sample_values, label_value, child_index):
            if len(self.children) <= child_index:  # create a child
                ...
                self.children.append(BatchTree.Node(self.tree_parent, [-1 if sample_values is None else sample_values],
                                                    [-1 if label_value is None else label_value],
                                                    [len(self.samples_values) - 1], self))
            else:
                self.children[child_index].add_sample(-1 if sample_values is None else sample_values,
                                                      -1 if label_value is None else label_value,
                                                      len(self.samples_values) - 1)
    ...
    def build_batch_tree_sample(self):
        ...
        while q:
            ...
            observable = np.array(node.samples_values)[
                np.array(node.samples_values) >= 0]  # .reshape((1, len(node.samples_values)))
            ...
            if node.children:
                ...
                c = node.children[0]
                childs_info = np.arange(len(c.samples_values) * len(node.children)).reshape(len(c.samples_values), len(
                    node.children)).transpose().reshape(-1) + batch_levels[level]["childs_transpose_scatter_offset"]
                batch_levels[level]["scatter_in"].append(
                    np.array(c.scatter_indices) + batch_levels[level]["scatter_in_offset"])
        ...
        levels = reversed(range(max_level))
        for l, i in zip(range(max_level), levels):
            ...
            observables = np.concatenate(
                [observables, np.concatenate(level_dict["observables"], axis=0).astype(dtype=np.int32)], axis=0)
            ...
            scatter_out = np.concatenate(
                [scatter_out, np.concatenate(level_dict["scatter_out"], axis=0).astype(dtype=np.int32)], axis=0)
            if l > 0:
                childs_transpose_scatter = np.concatenate([childs_transpose_scatter,
                                                           np.concatenate(level_dict["childs_transpose_scatter"],
                                                                          axis=0).astype(dtype=np.int32)], axis=0)
            ...
            labels = np.concatenate([labels, np.concatenate(level_dict["labels"], axis=0).astype(dtype=np.int32)],
                                    axis=0)
